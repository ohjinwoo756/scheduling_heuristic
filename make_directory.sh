...
main='src/main_for_directory.py'
heuristic='GA'
...
for ((n1_idx=0; n1_idx < ${#networks[@]}; n1_idx++)); do # for all networks
	for ((p_idx=0; p_idx < ${#pe_config[@]}; p_idx++)); do # for all pe  configuration
		for ((c_idx=0; c_idx < ${#cpu_config[@]}; c_idx++)); do # for all cpu configuration
			...
			echo "==========================================================="
			python ${main} -s ${heuristic} -n ${networks[n1_idx]} -o ${objective} -r ${pe_config[p_idx]} -c ${cpu_config[c_idx]} &
	...
	for ((n2_idx=0; n2_idx < ${#networks[@]}; n2_idx++)); do # for priority 2
		...
		for ((p_idx=0; p_idx < ${#pe_config[@]}; p_idx++)); do # for all pe  configuration
			for ((c_idx=0; c_idx < ${#cpu_config[@]}; c_idx++)); do # for all cpu configuration
				...
				echo ${elapsed_of_n1} ${elapsed_of_n2} ${deadline}
				echo "==============================================================================="
				python ${main} -s ${heuristic} -n ${networks[n1_idx]} ${networks[n2_idx]} -o ${objective} ${objective} -t ${constraint} ${constraint} -d ${deadline} ${deadline} -r ${pe_config[p_idx]} -c ${cpu_config[c_idx]} &
		...
		for ((n3_idx=0; n3_idx < ${#networks[@]}; n3_idx++)); do # for priority 3
			...
			for ((p_idx=0; p_idx < ${#pe_config[@]}; p_idx++)); do # for all pe  configuration
				for ((c_idx=0; c_idx < ${#cpu_config[@]}; c_idx++)); do # for all cpu configuration
					...
					echo "==============================================================================="
					python ${main} -s ${heuristic} -n ${networks[n1_idx]} ${networks[n2_idx]} ${networks[n3_idx]} -o ${objective} ${objective} ${objective} -t ${constraint} ${constraint} ${constraint} -d ${deadline} ${deadline} ${deadline} -r ${pe_config[p_idx]} -c ${cpu_config[c_idx]} &
