	...
	for ((n2_idx=0; n2_idx < ${#networks[@]}; n2_idx++)); do # for priority 2
		...
		for ((p_idx=0; p_idx < ${#pe_config[@]}; p_idx++)); do # for all pe  configuration
			for ((c_idx=0; c_idx < ${#cpu_config[@]}; c_idx++)); do # for all cpu configuration
				...
				echo "==============================================================================="
				python src/main.py -s ${heuristic} -n ${networks[n1_idx]} ${networks[n2_idx]} -o ${objective} ${objective} -t ${constraint} ${constraint} -d ${deadline} ${deadline} -r ${pe_config[p_idx]} -c ${cpu_config[c_idx]} > ${heuristic}_${networks[n1_idx]}_${networks[n2_idx]}_${deadline}_${deadline}_${pe_config_c[p_idx]}_${cpu_config[c_idx]}.log &

				progress=`expr $progress + 1`
				if [ `expr $progress % 7` == 0 ]; then
					wait
				fi
